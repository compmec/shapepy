...
def open_spline(vertices: Tuple[Tuple[float, float], ...]) -> SplineOpenCurve:
    knotvector = [0] + list(range(len(vertices))) + [len(vertices) - 1]
    knotvector = KnotVector(knotvector, 1)
    ctrlpoints = tuple(vertices)
...
def closed_spline(
    ...
    knotvector = [0] + list(range(len(vertices) + 1)) + [len(vertices)]
    knotvector = KnotVector(knotvector, 1)
    ctrlpoints = list(vertices) + [vertices[0]]
...
class TestOpenCurve:
    ...
    def test_compare(self):
        vertices = [(0, 0), (1, 0), (1, 1)]
        curvea = open_spline(vertices)
        curveb = open_spline(vertices)
    ...
    def test_section(self):
        ...
        vertices = [(half, 0), (1, 0), (1, 1)]
        curveb = open_spline(vertices)
        assert curvea.section(half, 2) == curveb
        vertices = [(0, 0), (1, 0), (1, half)]
        curveb = open_spline(vertices)
        assert curvea.section(0, 1 + half) == curveb
        vertices = [(half, 0), (1, 0), (1, half)]
        curveb = open_spline(vertices)
        assert curvea.section(half, 1 + half) == curveb
...
class TestClosedCurve:
    ...
    def test_compare(self):
        vertices = [(0, 0), (1, 0), (1, 1), (0, 1)]
        curvea = closed_spline(vertices)
        curveb = closed_spline(vertices)
        assert curvea == curvea
        assert curveb == curveb
        assert curvea == curveb
        assert id(curvea) != id(curveb)
    ...
    def test_section(self):
        ...
        vertices = [(half, 0), (1, 0), (1, 1), (0, 1), (0, half)]
        curveb = open_spline(vertices)
        curvec = curvea.section(half, 3 + half)
        assert isinstance(curvec, SplineOpenCurve)
